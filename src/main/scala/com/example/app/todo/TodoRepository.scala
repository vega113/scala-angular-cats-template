...
object TodoRepository {
  ...
  private final class DoobieTodoRepository[F[_]: Async](xa: Transactor[F])
    ...
    override def create(userId: UUID, create: TodoCreate): F[Todo] =
      for
        ...
        _ <-
          sql"INSERT INTO todos (id, user_id, title, description, due_date, completed, created_at, updated_at) VALUES ($id, $userId, ${create.title}, ${create.description}, ${create.dueDate}, false, $now, $now)".update.run
            .transact(xa)
      yield todo
    ...
    override def list(
      ...
      val filtered = completed match
        ...
        case None => base
      (filtered ++ fr"ORDER BY created_at DESC LIMIT $limit OFFSET $offset")
        .query[Todo]
        .to[List]
        .transact(xa)
    }
    ...
    override def update(userId: UUID, id: UUID, update: TodoUpdate): F[Option[Todo]] = {
      ...
      for
        _ <-
          if (setFragments.isEmpty) Async[F].unit
          else {
            val assignments = setFragments.tail.foldLeft(setFragments.head) { (acc, frag) =>
              acc ++ fr", " ++ frag
            }
            val updateFragment =
              fr"UPDATE todos SET " ++ assignments ++ fr" WHERE user_id = $userId AND id = $id"
