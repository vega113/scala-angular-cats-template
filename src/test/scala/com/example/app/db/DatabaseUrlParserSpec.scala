...
class DatabaseUrlParserSpec extends FunSuite:
  private val baseConfig = DbConfig(url = None, user = None, password = None, schema = None, maxPoolSize = 8)

  test("normalize converts postgres URL to JDBC and extracts credentials") {
    val config = baseConfig.copy(url = Some("postgres://user:pass@db.example.com:5432/app?sslmode=require"))
